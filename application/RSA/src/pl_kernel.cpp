...
void merge_phase2(hls::stream<ap_uint<DDR_BITS>> &final_merge_out, hls::stream<ap_uint<DDR_BITS>> &serialize_out, const uint32_t batch, const uint32_t repeat){
    // generate
    uint16_t shift[64] = { 0, 120, 240, 360, 480, 88, 208, 328, 448, 56, 176, 296, 416, 24, 144, 264, 384, 504, 112, 232, 352, 472, 80, 200, 320, 440, 48, 168, 288, 408, 16, 136, 256, 376, 496, 104, 224, 344, 464, 72, 192, 312, 432, 40, 160, 280, 400, 8, 128, 248, 368, 488, 96, 216, 336, 456, 64, 184, 304, 424, 32, 152, 272, 392 };
...
void pl_kernel(ap_uint<DDR_BITS> *data_in, ap_uint<DDR_BITS> *data_out, ap_uint<DDR_BITS> *mem_exp, ap_uint<DDR_BITS> *mem_n, ap_uint<DDR_BITS> *mem_nInv, ap_uint<DDR_BITS> *rMod, ap_uint<DDR_BITS> *rMod_x_1, uint32_t e_bits, uint32_t batch, uint32_t repeat, const bool block_ddr \
            , hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_a0, hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_a1 \
            , hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_b0, hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_b1, hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_b2 \
            , hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_c0, hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_c1, hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_c2, hls::stream<qdma_axis<PLIO_BITS, 0, 0, 0> > &p_0_c3){
...
#pragma HLS STREAM variable = rsa0_partial_s depth = 128
#pragma HLS STREAM variable = rsa0_backup_s depth = 128
    // mmm
...
#pragma HLS STREAM variable = rsa0_mmm_partial_res_s depth = 128
#pragma HLS STREAM variable = rsa0_mmm_add_shift_in0_s depth = 128
    ...
    hls::stream<ap_uint<PLIO_BITS>> rsa0_pl_a0_s;
    hls::stream<ap_uint<PLIO_BITS>> rsa0_pl_a1_s;


#pragma HLS STREAM variable = rsa0_pl_a0_s depth = 68
#pragma HLS STREAM variable = rsa0_pl_a1_s depth = 68


    hls::stream<ap_uint<PLIO_BITS>> rsa0_pl_pb0_s;


    hls::stream<ap_uint<PLIO_BITS>> rsa0_pl_b0_s;
    hls::stream<ap_uint<PLIO_BITS>> rsa0_pl_b1_s;
    hls::stream<ap_uint<PLIO_BITS>> rsa0_pl_b2_s;


#pragma HLS STREAM variable = rsa0_pl_b0_s depth = 68
#pragma HLS STREAM variable = rsa0_pl_b1_s depth = 68
#pragma HLS STREAM variable = rsa0_pl_b2_s depth = 68


    hls::stream<ap_uint<DDR_BITS>> rsa0_merge0_c0;
    hls::stream<ap_uint<DDR_BITS>> rsa0_merge0_c1;
    hls::stream<ap_uint<DDR_BITS>> rsa0_merge0_c2;
    hls::stream<ap_uint<DDR_BITS>> rsa0_merge0_c3;


#pragma HLS STREAM variable = rsa0_merge0_c0 depth = 34
#pragma HLS STREAM variable = rsa0_merge0_c1 depth = 34
#pragma HLS STREAM variable = rsa0_merge0_c2 depth = 34
#pragma HLS STREAM variable = rsa0_merge0_c3 depth = 34




    hls::stream<ap_uint<DDR_BITS>> rsa0_merge0_pc0;
    hls::stream<ap_uint<DDR_BITS>> rsa0_merge0_pc1;
...
#pragma HLS dataflow
    ...
    split_input_b(rsa0_reorganized_b_s, rsa0_pl_b0_s, rsa0_pl_pb0_s, AIE_H / (PLIO_BITS / 32), AIE_W / (PLIO_BITS / 32), AIE_W * 2 / (PLIO_BITS / 32), batch, repeat * (e_bits * 2 + 1) * 3);

    split_input_b(rsa0_pl_pb0_s, rsa0_pl_b1_s, rsa0_pl_b2_s, AIE_H / (PLIO_BITS / 32), AIE_W / (PLIO_BITS / 32), AIE_W / (PLIO_BITS / 32), batch, repeat * (e_bits * 2 + 1) * 3);


    send_plio(rsa0_pl_b0_s, p_0_b0, (AIE_W + AIE_H) / (PLIO_BITS / 32), batch, repeat * (e_bits * 2 + 1) * 3);
    send_plio(rsa0_pl_b1_s, p_0_b1, (AIE_W + AIE_H) / (PLIO_BITS / 32), batch, repeat * (e_bits * 2 + 1) * 3);
    send_plio(rsa0_pl_b2_s, p_0_b2, (AIE_W + AIE_H) / (PLIO_BITS / 32), batch, repeat * (e_bits * 2 + 1) * 3);


    receive_plio(rsa0_merge0_c0, p_0_c0, batch, repeat * (e_bits * 2 + 1) * 3);
    receive_plio(rsa0_merge0_c1, p_0_c1, batch, repeat * (e_bits * 2 + 1) * 3);
    receive_plio(rsa0_merge0_c2, p_0_c2, batch, repeat * (e_bits * 2 + 1) * 3);
    receive_plio(rsa0_merge0_c3, p_0_c3, batch, repeat * (e_bits * 2 + 1) * 3);


    serialize_merge_stream(rsa0_serialized_out, rsa0_merge0_c0, rsa0_merge0_pc0, AIE_OUTPUT_CHUNK, AIE_OUTPUT_CHUNK * 3, batch, repeat * (e_bits * 2 + 1) * 3);

    serialize_merge_stream(rsa0_merge0_pc0, rsa0_merge0_c1, rsa0_merge0_pc1, AIE_OUTPUT_CHUNK, AIE_OUTPUT_CHUNK * 2, batch, repeat * (e_bits * 2 + 1) * 3);
    serialize_merge_stream(rsa0_merge0_pc1, rsa0_merge0_c2, rsa0_merge0_c3, AIE_OUTPUT_CHUNK, AIE_OUTPUT_CHUNK, batch, repeat * (e_bits * 2 + 1) * 3);
