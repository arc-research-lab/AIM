...
#     ############################################################
    ...
    for i in range(64):
        ...
        if r >= 512:
            r -= 512
        # shift = 
    code = (code.render(PLIO_A=int(AIE_ROW), PLIO_B=AIE_COL, OUTPUT_AIE=OUTPUT_AIE, DDR_BITS=DDR_BITS, AIE_H=AIE_H, AIE_W=AIE_W, shift2=shift[:-2]))
...
#     # host.cpp
    with open('tools/host.cpp.jinja') as f:
        ...
        code = (code.render(PARALLELISM=int(PARALLELISM), AIE_COL=AIE_COL, OUTPUT_AIE=OUTPUT_AIE, AIE_ROW=AIE_ROW))
